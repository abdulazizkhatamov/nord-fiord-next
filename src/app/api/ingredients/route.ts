...
// Merge schemas
const ingredientsQuerySchema = paginationAndSortingSchema.extend(
  ingredientsFilterSchema.shape
);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const query = ingredientsQuerySchema.parse(searchParams);
    ...
    // Count total rows
    const rowCount = await prisma.ingredient.count({ where });
