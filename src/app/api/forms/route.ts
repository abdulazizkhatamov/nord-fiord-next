...
// GET - Filters schema
const formsFilterSchema = z.object({
  name: z.string().optional(),
});

// Merge schemas
const formsQuerySchema = paginationAndSortingSchema.extend(
  formsFilterSchema.shape
);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const query = formsQuerySchema.parse(searchParams);
    ...
    // Count total rows
    const rowCount = await prisma.form.count({ where });
