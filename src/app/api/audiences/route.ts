...
// GET - Filters schema
const audiencesFilterSchema = z.object({
  name: z.string().optional(),
});

// Merge schemas
const audiencesQuerySchema = paginationAndSortingSchema.extend(
  audiencesFilterSchema.shape
);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const query = audiencesQuerySchema.parse(searchParams);
    ...
    // Count total rows
    const rowCount = await prisma.audience.count({ where });
